...
namespace DotNetty.Handlers.Tls
{
    ...
    {
        static readonly IInternalLogger Logger = InternalLoggerFactory.GetInstance<TlsHandler>();
        ...
        public TlsHandler(TlsSettings settings, IEventExecutor executor = default)
            : this(stream => new SslStream(stream, true), settings, executor)
        {
        }

        public TlsHandler(Func<Stream, SslStream> sslStreamFactory, TlsSettings settings, IEventExecutor executor = default)
        {
            ...
            this.mediationStream = new MediationStream(this, executor);
        ...
        {
            // Logger.Debug($"{nameof(this.Decode)} Tid: {Thread.CurrentThread.ManagedThreadId}");
        ...
        {
            // Logger.Debug($"{nameof(this.Read)} Tid: {Thread.CurrentThread.ManagedThreadId}");
